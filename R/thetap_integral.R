thetap_integral <- function(data, A, W, Z, M, fit_or, fit_pz, fit_pm1, ap = 1, as = 0, aj) {
...
thetap_integral1 <- function(data, A, W, Z, M, fit_or, fit_pm1, ap = 1, as = 0) {
  num_class_M <- nrow(unique(data[, c(M), with = FALSE]))
...
thetap_integral2 <- function(data, A, W, Z, M, fit_or, fit_pz, as = 0, aj) {
  num_class_Z <- nrow(unique(data[, c(Z), with = FALSE]))
