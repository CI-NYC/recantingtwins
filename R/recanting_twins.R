...
recanting_twins <- function(data, W, A, Z, M, Y,
    ...
    fit_pz <- pz_disc(data, A, W, Z, .control)
    # Fit the density of M conditional on A,Z,W: P(M|a,Z,W)
    fit_pm2 <- pm2_disc(data, A, W, M, Z, .control)
    ...
    fit_pm1 <- pm1_disc(data, A, W, M, .control)
    # Estimate integrals: E_h, E_h1, E_h2
    Eh_thetap1 <- thetap_integral_disc(data, A, Z, M, fit_or$fit, fit_pz$fit, fit_pm1$fit, ap = 1, as = 0, aj = 1)
    Eh1_thetap1 <- thetap_integral1_disc(data, A, M, fit_or$fit, fit_pm1$fit, ap = 1, as = 0)
    Eh2_thetap1 <- thetap_integral2_disc(data, A, Z, fit_or$fit, fit_pz$fit, as = 0, aj = 1)
    ...
    # theta' 2 ----------------------------------------------------------------
    # Estimate integrals: E_h, E_h1, E_h2
    Eh_thetap2 <- thetap_integral_disc(data, A, Z, M, fit_or$fit, fit_pz$fit, fit_pm1$fit, ap = 1, as = 0, aj = 0)
    Eh1_thetap2 <- Eh1_thetap1
    Eh2_thetap2 <- thetap_integral2_disc(data, A, Z, fit_or$fit, fit_pz$fit, as = 0, aj = 0)
    ...
    # theta'' 3 ---------------------------------------------------------------
    Eh_theta3_2p <- theta2p_integral_disc(data, A, Z, M, fit_or$fit, fit_pz$fit, fit_pm2$fit, ap = 1, as = 0)
    Eh1_theta3_2p <- theta2p_integral1_disc(data, A, Z, M, fit_or$fit, fit_pz$fit, fit_pm2$fit, ap = 1, as = 0)
    Eh2_theta3_2p <- theta2p_integral2_disc(data, A, Z, M, fit_or$fit, fit_pz$fit, fit_pm2$fit, ap = 1, as = 0)
    ...
    Eh_theta3 <- theta3_integral_disc(data, A, Z, M, fit_or$fit, fit_pz$fit, fit_pm2$fit, ap = 1, as = 0)
    Eh1_theta3 <- theta3_integral1_disc(data, A, M, fit_or$fit, fit_pm2$fit, ap = 1, as = 0)
