pm1 <- function(data, A, W, M, .control) {
  ...
  fit <- mlr3superlearner(data[,c(A, W, M), with = FALSE],
                          target = M,
                          library = .control$.pm1_learners,
                          outcome_type = "multiclass",
                          folds = .control$.pm1_folds)

  data_temp <- assign_value(data, A, 0)

  `P(M|0,W)` <- predict(fit, data_temp[,c(A, W), with = F], discrete = F)

  data_temp <- assign_value(data, A, 1)

  `P(M|1,W)` <- predict(fit, data_temp[,c(A, W), with = F], discrete = F)
...
pm2 <- function(data, A, W, M, Z, .control) {
  ...
  fit <- mlr3superlearner(data[,c(A, W, Z, M), with = FALSE],
                          target = M,
                          library = .control$.pm2_learners,
                          outcome_type = "multiclass",
                          folds = .control$.pm2_folds)

  data_temp <- assign_value(data, A, 0)

  `P(M|0,Z,W)` <- predict(fit, data_temp[,c(A, W, Z), with = F], discrete = F)

  data_temp <- assign_value(data, A, 1)

  `P(M|1,Z,W)` <- predict(fit, data_temp[,c(A, W, Z), with = F], discrete = F)
